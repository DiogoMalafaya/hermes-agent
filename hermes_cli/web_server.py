...
def _mcp_server_summary(name: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # OAuth servers surface whether a cached token exists so the dashboard can
    # show a "tokens cached" vs "not authenticated" badge and drive the
    # browser-based OAuth flow. None for non-OAuth servers.
    has_tokens: Optional[bool] = None
    if cfg.get("auth") == "oauth":
        try:
            from tools.mcp_oauth import HermesTokenStorage

            has_tokens = HermesTokenStorage(name).has_cached_tokens()
        except Exception:
            has_tokens = False
    return {
        "name": name,
