        ...
        while True:
            try:
                ...
                if not self._ready.is_set():
                    ...
                    # single clean message so other servers continue. Also skip
                    # for any other initial auth failure (upstream behavior).
                    try:
                        from tools.mcp_oauth import OAuthNonInteractiveError
                        non_interactive_oauth = isinstance(exc, OAuthNonInteractiveError)
                    except ImportError:
                        non_interactive_oauth = False
                    if non_interactive_oauth or _is_auth_error(exc):
                        logger.warning(
                            "MCP server '%s' skipped at startup (OAuth could not "
                            "complete, not retrying): %s",
                            self.name, exc,
                        )
                        self._error = exc
                        self._ready.set()
